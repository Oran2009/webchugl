    ...
    var CK = {
        ...
        loadFile: function(url, vfsPath) {
            ...
            return fetch(url)
                .then(function(response) {
                    ...
                    return isChugin ? response.arrayBuffer() : response.text();
                })
                .then(function(data) {
                    _ckDefer(function() {
                        _ensureVfsDir(vfsPath);
                        _module.FS.writeFile(vfsPath, isChugin ? new Uint8Array(data) : data);
        ...
        runZip: function(url, mainFile) {
            ...
            return fetch(url)
                ...
                .then(function() {
                    return _module.ccall('ck_run_file', 'number', ['string'], [mainFile]);
                });
        },
        ...
        loadPackage: function(name, version, url) {
            ...
            // Load JSZip if needed (reuse pattern from runZip)
        ...
        return Promise.all([chuginPromise, gpuPromise]).then(function(results) {
            ...
            return adapter.requestDevice().then(function(device) {
                ...
                for (var i = 0; i < _pendingChuginPaths.length; i++) {
                    _loadChuginFromVfs(_pendingChuginPaths[i]);
                }
